...
function drawGrid() {
	...
	gridLayer.add(
		new Konva.Rect({
			...
			stroke: "#94a3b8", // más tenue que la cuadrícula
	...
	for (let r = GRID_SIZE * 5; r < maxRadius; r += GRID_SIZE * 5) {
		gridLayer.add(
			new Konva.Circle({
				...
				y: centerY,
				radius: r,
				stroke: "#eee", // más claro que las líneas del grid
				strokeWidth: 3,
				dash: [12, 12], // opcional: lo hace más sutil
			})
		);
	}
...
async function addPropertyToEntity(entity) {
	...
	const { value: formValues } = await Swal.fire({
		...
		preConfirm: () => {
			...
			if (name.length > 40) {
				Swal.showValidationMessage("El nombre no puede tener más de 40 caracteres");
				return false;
			}
			return {
				name: name.replaceAll(" ", "_"),
...
function deleteProperty(property, entity) {
	Swal.fire({
		...
		text: `Se eliminará la propiedad "${property.name}"`,
		icon: "warning",
		showCancelButton: true,
...
function deleteEntity(entity) {
	Swal.fire({
		...
		text: `Se eliminará la entidad "${entity.name}" y todas sus propiedades`,
		icon: "warning",
		showCancelButton: true,
		confirmButtonText: "Eliminar",
		cancelButtonText: "Cancelar",
...
async function createRelationship() {
	if (entities.length === 0) {
		Swal.fire({
			icon: "warning",
			title: "Sin entidades",
			text: "Necesitas al menos una entidad para crear una relación",
		});
		return;
	}
...
function updateRelationshipLines(relationship) {
	...
	function positionCardinality(cardGroup, entityPos, entityWidth, entityHeight, side, isSelfRelation) {
		...
		if (isSelfRelation) {
			dy = 0;
			dx = side === "left" ? -diamondWidth / 2 - 40 : diamondWidth / 2 + 40;
		} else {
...
function deleteRelationship(relationship) {
	Swal.fire({
		...
		text: `Se eliminará la relación "${relationship.name}"`,
		icon: "warning",
		showCancelButton: true,
		confirmButtonText: "Eliminar",
		cancelButtonText: "Cancelar",
...
function showEmptySpaceContextMenu(x, y) {
	...
	menu.innerHTML = `
			     ...
			     <div class="context-menu-item" data-action="relationship">
			         <i class="fas fa-link"></i> Nueva Relación
			     </div>
			 `;
...
function checkErrors() {
	...
	entities.forEach((entity) => {
		...
		if (pkCount > 1) {
			errors.push({
				type: "error",
				title: "Múltiples Claves Primarias",
	...
	relationships.forEach((rel) => {
		...
		if (!fromEntity) {
			errors.push({
				...
				message: `La relación "${rel.name}" referencia una entidad 'from' que no existe.`,
			});
		}
		if (!toEntity) {
			errors.push({
				type: "error",
				title: "Entidad Faltante",
				message: `La relación "${rel.name}" referencia una entidad 'to' que no existe.`,
			});
		}
		...
		if (!rel.cardinality || !rel.cardinality.includes(":")) {
			errors.push({
				...
				message: `La relación "${rel.name}" no tiene cardinalidad correcta.`,
			});
		}
		...
		if (fromEntity && toEntity && fromEntity.id === toEntity.id && (!rel.cardinality || !rel.cardinality.includes(":"))) {
			errors.push({
				type: "warning",
				title: "Relación Propia Sin Cardinalidad",
				message: `La relación "${rel.name}" sobre la misma entidad no tiene cardinalidad definida.`,
			});
		}
	...
	properties.forEach((p) => {
		if (!entities.find((e) => e.id === p.entity.id)) {
			errors.push({
				type: "error",
				title: "Propiedad Huérfana",
				message: `La propiedad "${p.property.name}" no pertenece a ninguna entidad válida.`,
			});
		}
	});
	...
	entities.forEach((e) => {
		...
		if (!hasRel) {
			errors.push({
				...
				message: `La entidad "${e.name}" no participa en ninguna relación.`,
			});
		}
	});
...
function exportToSQL() {
	let sql = "-- JaimeDiagramas - Exportación SQL\n";
